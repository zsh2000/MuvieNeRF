...
#        print(torch.isnan(feats).any(), torch.isnan(affine_mats_inv).any(), torch.isnan(affine_mats).any(), torch.isnan(depth_values).any(), idx, spikes)
...
#            print(torch.isnan(warped_volume).any(), torch.isnan(grid).any(), torch.isnan(src_feats[i]).any(), torch.isnan(proj_mat).any(), torch.isnan(depth_values).any(), "homo")
        ...
        for l in reversed(range(self.levels)):  # (2, 1, 0)
            ...
            if l == self.levels - 1:  # coarsest level
                ...
                if self.use_depth:
                    ...
                    spikes = (
                        torch.arange(D).view(1, 1, -1, 1, 1).to(device)
...
#            print(torch.isnan(v_feat_l).any(), "before")
...
#            print(torch.isnan(depth_prob).any(), torch.isnan(depth_l).any(), "depth")
#            print(torch.isnan(v_feat_l).any(), "after")
...
#        print(imgs.shape, "im_shape")
        ...
        for i in range(0, V):
            permuted_idx = torch.tensor(closest_idxs[0, i]).to(device)
...
#        print(torch.isnan(feats_vol[f"level_0"]).any(), "level0")
